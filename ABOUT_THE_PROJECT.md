...
$$\text{Consistency} = 100 \times \left(1 - \frac{\sigma_{laptimes}}{\mu_{laptimes}}\right)$$

Where $\sigma$ is the standard deviation and $\mu$ is the mean lap time.

**Gear Efficiency Metric:**
$$\text{Efficiency} = \frac{\text{Optimal Gear Usage}}{\text{Total Gear Changes}} \times 100$$
...
Instead of recalculating lap statistics on every dashboard load:
$$\text{Pre-compute: } \bar{v} = \frac{1}{n}\sum_{i=1}^{n} v_i$$
...
**Validation Example:**
$$\text{Valid if: } 0 < \text{speed} < 200 \text{ mph AND } 1 \leq \text{gear} \leq 6$$
...
$$\bar{v}_{\text{sector}} = \frac{\sum_{i=1}^{n} v_i}{n}$$

**Lap Time Consistency:**
$$\text{Consistency\%} = 100 \times \left(1 - \frac{\text{StdDev}(\text{LapTimes})}{\text{Mean}(\text{LapTimes})}\right)$$

**Gear Usage Efficiency:**
$$\text{Efficiency} = \frac{\text{Time in Optimal Gear}}{\text{Total Lap Time}} \times 100$$
